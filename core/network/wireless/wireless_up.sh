...
if test $# -ne 1; then
 echo "Script needs for 1 arguments but actual $#!"; exit 11; fi
...
for it in NI_NAME; do
 if test -z "${!it}"; then echo "$it is empty!"; exit 102; fi; done

if [ ! -s "/etc/wpa_supplicant.${SSID}.conf" ]; then
 echo "WPA supplicant conf for ssid \"$SSID\" does not exist!"; exit 13; fi
...
case "$NI_STATE" in
 ...
 "$NI_STATE_DOWN")
  ...
  if test $? -ne 0; then
   echo "Network interface \"$NI_NAME\" up error!"; exit 21; fi
 ;;
 *)
...
if test $? -ne 0; then
 echo "Connect to access point \"$SSID\" error!"; exit 31; fi

/usr/sbin/dhclient $NI_NAME
if test $? -ne 0; then
 echo 'dhclient error!'; exit 32; fi
