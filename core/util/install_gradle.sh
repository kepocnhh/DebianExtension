...
if test -d "/opt/gradle-${GRADLE_VERSION}"; then
 echo "Gradle ${GRADLE_VERSION} exists!"; exit 13
fi
...
if test $? -ne 0; then
 echo "Download gradle error!"; exit 21
fi
...
if test $? -ne 0; then
 echo "Download gradle checksum error!"; exit 22
fi
...
if test $? -ne 0; then
 echo "Unzip gradle error!"; exit 31
fi
rm /tmp/${FILE}
...
if test $? -ne 0; then
 echo "Running gradle error!"; exit 41
fi
