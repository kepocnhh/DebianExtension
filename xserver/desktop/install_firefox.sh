...
if test -d "/opt/mozilla/firefox-$ISSUER_VERSION"; then
...
if test $? -ne 0; then
 echo "Install lib error!"; exit 41
fi

BASE_URL="https://ftp.mozilla.org/pub/firefox/releases/$ISSUER_VERSION"
...
FILE="firefox-${ISSUER_VERSION}.tar.bz2"
rm /tmp/$FILE
curl -f "$BASE_URL/$DISTRIBUTION/$LANGUAGE/$FILE" -o /tmp/$FILE
if test $? -ne 0; then
 echo "Download $ISSUER error!"; exit 52
fi
...
if test $? -ne 0; then
 echo "Download $ISSUER signature error!"; exit 53
fi
...
if test $? -ne 0; then
 echo "Unzip $ISSUER error!"; exit 71
fi
...
mv /tmp/firefox "/opt/mozilla/firefox-$ISSUER_VERSION"
if test $? -ne 0; then
 echo "Install $ISSUER error!"; exit 81
fi
rm /tmp/$FILE
